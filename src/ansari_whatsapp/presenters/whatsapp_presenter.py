# WhatsApp Presenter for ansari-whatsapp
"""Presenter for handling WhatsApp interactions."""
...
async def extract_relevant_whatsapp_message_details(
    ...
    Args:
        body (dict[str, dict]): The JSON body of the incoming request.

    Returns:
        tuple[bool, bool, Optional[str], Optional[str], Optional[dict], Optional[str], Optional[int]]:
            A tuple of:
            (is_status, is_target_business_number, user_whatsapp_number, incoming_msg_type, incoming_msg_body, message_id, message_unix_time)
...
class WhatsAppPresenter:
    def __init__(
        ...
        """
        Initialize the WhatsApp presenter with user-specific details.
    ...
    async def _send_whatsapp_typing_indicator(self) -> None:
        """Sends a typing indicator to the WhatsApp recipient."""
        if not self.user_phone_num or not self.message_id:
    ...
    async def send_whatsapp_message(
        ...
        """
        Sends a message to the WhatsApp recipient.
    ...
    def _get_retention_time_in_seconds(self) -> int:
        """
        Get the retention time in seconds from settings.

        Returns:
            int: The retention time in seconds.
        """
        reten_hours = settings.WHATSAPP_CHAT_RETENTION_HOURS
